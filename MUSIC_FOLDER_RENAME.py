...
with open('detail.txt','a',encoding = 'UTF-8') as f:
    ...
    for folder_name in folder_name_list:
        ...
        for folder_folder_name in folder_folder_list:
            ...
            #check_name = 'Porter Robinson - Mirror myfreemp3.vip '
            print(os.path.join(name_fold,folder_folder_name))
            #
            #
            # 
            #
            ...
            if check != None:
                print("進if")
                print('regex : '+check.group(2))
                          #os.rename(os.path.join(name_fold,folder_folder_name),os.path.join(name_fold,check.group(1)))
                
            else:
                print('REGEX:'+folder_folder_name)
            '''


            split_str = 'myfreemp3.vip'
            check_name = folder_folder_name
            if split_str in check_name:
                new_name = check_name.split(split_str)
                #f.write(''.join(new_name))
                print("寫完ㄌ")
                os.rename(os.path.join(name_fold,folder_folder_name),str(name_fold)+'\\'+''.join(new_name))
            else:
                print('不用')
